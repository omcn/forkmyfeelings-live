...
export default function ProfilePage() {
  ...
  const refreshIncomingRequests = async () => {
    ...
    try {
      ...
      setIncomingCount(data?.length || 0);
    } catch (err) {
      console.error("🔁 Unexpected error in refreshIncomingRequests:", err);
    }
  };
    ...
    const getProfile = async () => {
      try {
        ...
        if (profileError || !profile) {
          console.error("⚠️ Could not load profile:", profileError?.message);
          setLoading(false);
          return;
        }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-rose-100 to-orange-100 flex flex-col items-center justify-center px-6 py-12">
      ...
      {showFindFriends && (
        <FindFriends profile={profile} onClose={() => setShowFindFriends(false)} />
      )}
